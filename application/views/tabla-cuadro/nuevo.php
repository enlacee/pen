...
<?php endif;?>
<h3>Registrar en Cuadro</h3>
<?php 
$atributes = array ('method' => 'post', 'name'=>'form-tabla-cuadro');
echo form_open("/tabla_cuadro/nuevo/" . $idCuadro, $atributes); ?>
<input type="hidden" name="id_cuadro" id="id_cuadro" value="<?php echo $idCuadro ?>"/>
<!-- inicio form -->
<?php foreach ($objCuadro as $indice => $arreglo): ?>
    <?php foreach ($arreglo as $key => $obj):?>
        <?php if (Variable_model::TIPO_LISTA_STRING == $obj->tipo_variable): ?>
            <?php $this->load->view('tabla-cuadro/form/_lista', array('obj' => $obj)); ?>            
        <?php else: ?>
            <?php $this->load->view('tabla-cuadro/form/_cadena', array('obj' => $obj)); ?>
        <?php endif; ?>        
    <?php endforeach; ?>
<?php endforeach;?>
